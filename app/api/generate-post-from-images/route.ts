import OpenAI from "openai";
import { readFile } from "fs/promises";
import path from "path";

function mdHeadingsToHtml(content: string): string {
  return content
    .replace(/^### (.+)$/gm, '<h3 style="font-size:20px;font-weight:900;margin:12px 0 6px">$1</h3>')
    .replace(/^## (.+)$/gm, '<h2 style="font-size:22px;font-weight:900;margin:16px 0 8px">$1</h2>')
    .replace(/^# (.+)$/gm, '<h1 style="font-size:28px;font-weight:900;margin:20px 0 10px">$1</h1>');
}

const client = new OpenAI({
  apiKey: process.env.OPENAI_API_KEY,
});

const branchInfo: any = {
  철산점: {
    area: "경기도 광명시 철산동",
    hours: "월-금 14:00~23:00, 토·일 14:00~18:00, 공휴일 휴무",
    note: "광명 철산동에서 복싱 입문, 다이어트, 체력 향상, 스트레스 해소를 편하게 시작할 수 있는 운영 중인 체육관",
    seoKeywords: ["철산 복싱", "광명 복싱", "철산역 복싱", "철산 다이어트"],
  },
  목동점: {
    area: "서울시 양천구 목동",
    hours: "월-금 14:00~24:00, 토 11:00~16:00",
    note: "키즈/어린이 회원이 특히 많고, 학생·여성·직장인 회원도 많은 밝고 편한 분위기",
    seoKeywords: ["목동 복싱", "양천구 복싱", "목동 키즈복싱", "목동 어린이복싱"],
  },
  신정점: {
    area: "서울시 양천구 신정동",
    hours: "월-금 10:00~24:00, 토 10:00~16:00",
    note: "키즈/어린이 회원이 특히 많고 직장인·주부·초보 회원도 많음",
    seoKeywords: ["신정 복싱", "신정동 복싱", "신정 키즈복싱", "양천구 어린이복싱"],
  },
  개봉점: {
    area: "서울시 구로구 개봉동",
    hours: "월-금 13:00~23:00",
    note: "동네 주민 중심으로 어린이·키즈 회원도 있고 직장인·여성·초보 회원도 골고루 있는 친근한 분위기",
    seoKeywords: ["개봉 복싱", "개봉동 복싱", "구로 복싱", "개봉 키즈복싱"],
  },
  영등포점: {
    area: "서울시 영등포구",
    hours: "월-금 13:00~23:00",
    note: "퇴근 후 오는 직장인 회원이 많고 키즈·어린이 회원도 일부 있음",
    seoKeywords: ["영등포 복싱", "영등포구 복싱", "영등포 다이어트", "영등포 직장인복싱"],
  },
};

export async function POST(request: Request) {
  try {
    const { imageUrls, branch_name } = await request.json();

    if (!imageUrls || imageUrls.length === 0) {
      return Response.json({ ok: false, message: "사진을 먼저 업로드해주세요." }, { status: 400 });
    }

    const branch = branchInfo[branch_name] || {};
    const seoKeywords = branch.seoKeywords || ["복싱", "다이어트 복싱", "복싱장"];

    const angles = [
      "오늘 수업 현장을 체육관 관장 시각으로 생생하게 전달",
      "초보자가 자주 묻는 질문에 체육관이 직접 답해주는 형식",
      "운동 효과보다 '분위기·사람'에 초점을 맞춘 현장 소개",
      "키즈·어린이 수업 현장을 부모에게 설명하는 체육관 블로그",
      "다이어트 목적으로 복싱을 선택해야 하는 이유를 체육관이 설명",
      "여성 회원이 편하게 다닐 수 있는 이유를 체육관 입장에서 소개",
      "직장인·퇴근 후 회원을 위한 수업 운영 방식 소개",
      "기초 체력 향상과 스트레스 해소 두 마리 토끼를 잡는 현장",
      "오늘 수업에서 회원들이 집중했던 동작·훈련 중심 현장 보고",
      "처음 오는 분들을 위한 체육관 첫 방문 가이드 형식",
    ];
    const angle = angles[Math.floor(Math.random() * angles.length)];

    const prompt = `
너는 네이버 지역 체육관 공식 블로그를 운영하는 마케터야.
글은 반드시 체육관(스트롱복싱 ${branch_name}) 관장 또는 운영자 입장에서 써야 해.
회원 후기 스타일("제가~", "처음엔 무서웠는데~", "~이야기") 절대 금지.

브랜드명: 스트롱복싱
현재 작성 지점: ${branch_name}

지점 정보:
- 지역: ${branch.area}
- 운영시간: ${branch.hours}
- 분위기: ${branch.note}
- 지점 SEO 키워드: ${seoKeywords.join(", ")}

위 사진들을 보고 오늘 어떤 수업/운동 현장이었는지 파악한 뒤, 그 내용을 중심으로 블로그 글을 작성해.
사진에 보이는 것들(아이들, 어른, 키즈 수업, 스파링, 미트 훈련, 다이어트 운동 등)을 자연스럽게 본문에 녹여넣어.

이번 글의 방향:
${angle}

[제목 규칙 — 매우 중요]
- 반드시 체육관 공식 블로그 제목 형식
- 좋은 예: "철산 복싱 다이어트, 이렇게 시작하세요 | 스트롱복싱 철산점"
- 좋은 예: "목동 키즈복싱 수업 현장 🥊 오늘도 열심히 달렸습니다"
- 좋은 예: "광명 복싱 입문반, 초보자도 첫날부터 바로 할 수 있어요"
- 나쁜 예 (절대 금지): "복싱을 처음 시작한 날의 이야기", "제가 복싱을 선택한 이유", "~후기", "~경험"

[핵심 규칙]
- 글 전체가 체육관이 방문객/잠재 회원에게 말하는 구조
- "도입 → 장점 나열 → 마무리" 같은 뻔한 구성 금지
- 동네 체육관의 친근하고 편안한 분위기
- 과장되거나 허위 느낌 금지
- 이모지는 문단 사이나 소제목 옆에 자연스럽게 3~5개 사용

[말투 규칙]
- "안녕하세요! 스트롱복싱 ${branch_name}입니다 😊" 같은 인사로 시작
- "~입니다", "~합니다" 격식체 유지
- 네이버 블로그 체육관 공식 글 특유의 밝고 친근한 격식체

[SEO 규칙]
- 제목에 지점 SEO 키워드 중 하나를 앞부분에 포함
- 본문 첫 문단에 지역명(${branch.area}) 자연스럽게 포함
- 반드시 소제목 3~5개 사용, 형식은 정확히 "## 소제목내용"
- 글 길이 최소 1200자
- 지점 SEO 키워드 중 2~3개 본문에 자연스럽게 포함
- 마지막 문단에 방문 상담이나 홈페이지로 자연스럽게 연결

[해시태그]
- 8~12개
- # 없이, 띄어쓰기 없는 태그
- 지역명, 스트롱복싱, 복싱입문, 다이어트복싱 포함

반드시 아래 JSON 형식으로만 답변해.
설명 문장, 코드블럭, markdown 없이 순수 JSON만 출력.

{
  "title": "블로그 제목",
  "slug": "영문-url-slug",
  "description": "블로그 목록에 보일 짧은 설명",
  "content": "본문 전체",
  "hashtags": ["스트롱복싱", "복싱입문", "다이어트복싱"]
}
`;

    // 이미지 URL → base64 data URI 변환 (OpenAI가 직접 서버 접근하면 막힐 수 있어서)
    const SUPPORTED_EXTS = new Set(["jpg", "jpeg", "png", "gif", "webp"]);
    const imageContents: any[] = [];
    for (const url of imageUrls) {
      try {
        // URL이 /uploads/... 형태면 서버 파일시스템에서 직접 읽음
        const localPath = url.startsWith("/") ? url : new URL(url).pathname;
        const filePath = path.join(process.cwd(), "public", localPath);
        const ext = (localPath.split(".").pop() || "jpg").toLowerCase().replace(/[^\w]/g, "");
        if (!SUPPORTED_EXTS.has(ext)) {
          console.warn(`지원하지 않는 형식 건너뜀: ${ext} (${url})`);
          continue;
        }
        const mimeType = ext === "jpg" || ext === "jpeg" ? "image/jpeg"
          : ext === "png" ? "image/png"
          : ext === "gif" ? "image/gif"
          : "image/webp";
        const buffer = await readFile(filePath);
        const base64 = buffer.toString("base64");
        imageContents.push({
          type: "image_url" as const,
          image_url: {
            url: `data:${mimeType};base64,${base64}`,
            detail: "low" as const,
          },
        });
      } catch (e: any) {
        console.warn(`이미지 읽기 실패 (건너뜀): ${url} — ${e?.message}`);
      }
    }

    if (imageContents.length === 0) {
      return Response.json({ ok: false, message: "사용 가능한 이미지가 없습니다. JPG/PNG/WEBP 형식만 지원해요." }, { status: 400 });
    }

    // Chat Completions API (vision 지원)
    const response = await client.chat.completions.create({
      model: "gpt-4o-mini",
      max_tokens: 4000,
      messages: [
        {
          role: "user",
          content: [
            ...imageContents,
            { type: "text" as const, text: prompt },
          ],
        },
      ],
    });

    const raw = response.choices[0].message.content?.trim() || "";
    const text = raw.replace(/^```(?:json)?\n?/, "").replace(/\n?```$/, "").trim();
    const json = JSON.parse(text);
    if (json.content) json.content = mdHeadingsToHtml(json.content);

    return Response.json({ ok: true, post: json });
  } catch (error: any) {
    const msg = error?.message || String(error);
    console.error("AI 이미지 기반 생성 오류:", msg);
    return Response.json({ ok: false, message: `AI 생성 오류: ${msg}` }, { status: 500 });
  }
}
