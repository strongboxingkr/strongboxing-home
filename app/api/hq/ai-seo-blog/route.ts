...
[글 시작 고정 문구 — 반드시 본문 맨 앞 두 줄]
안녕하세요 😊
스트롱복싱 ${branch_name}입니다💪

[말투 규칙]
- 네이버 블로그에 올리는 체육관 소식처럼 자연스럽고 친근하게
- "저희는 ~합니다"만 반복하지 않기
- 가르치는 척, 오글거리는 감성 문구 피하기
- 짧은 문장, 짧은 문단
- 초보자·여성·학생·직장인이 부담 없이 읽을 수 있는 말투
- 실제 수업 분위기를 옆에서 설명하듯 작성

[SEO 규칙]
- 본문 첫 문단에 메인 키워드 자연스럽게 포함
- 보조 키워드 1~2개만 자연스럽게 사용 (반복 금지)
- H2 소제목 3~5개 필수 (반드시 ## 형식 사용)
- 본문 전체 900~1200자

[절대 사용 금지]
- 무료체험, 무조건, 최고, 1등, 완벽한, 압도적인
- 인생이 바뀝니다, 누구나 선수처럼, 무조건 살 빠집니다
- 도보 5분, 역 바로 앞, 역세권, 실제와 다를 수 있는 거리/시간 표현
...
5. 수업 흐름 설명
6. 지점 분위기
7. 자연스러운 마무리: "복싱이 처음이라도 괜찮습니다. 스트롱복싱 ${branch_name}에서는 회원님 목적과 수준에 맞춰 차근차근 지도하고 있습니다."

[FAQ]
본문 뒤에 초보자가 실제로 궁금해할 질문 3개 (구체적으로)

[이미지 alt]
사진 내용과 지점명이 자연스럽게 들어가는 alt 3개 (키워드 나열 X, 실제 이미지 설명처럼)

반드시 아래 JSON 형식으로만 응답. 코드블럭·설명 없이 순수 JSON만 출력:
{
  "titles": ["제목후보1", "제목후보2", "제목후보3"],
  "best_title": "최종 추천 제목 (지역명+복싱장 포함, 50자 이내)",
  "description": "메타 description 80자 이내",
  "slug": "영어-소문자-하이픈-slug (지점영어명-boxing-주제 형태)",
  "body": "본문 전체 (## H2 소제목 포함, 고정 인사로 시작)",
  "faq": [
    {"q": "질문1", "a": "답변1"},
    {"q": "질문2", "a": "답변2"},
    {"q": "질문3", "a": "답변3"}
