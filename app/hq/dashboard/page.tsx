...
function Panel({ children, className = "" }: { children: React.ReactNode; className?: string }) {
  return (
    <div
      className={`rounded-3xl border transition-all duration-300 hover:-translate-y-0.5 hover:shadow-2xl ${className}`}
      style={{
...
export default function DashboardPage() {
  return (
    <div
      className="strong-hq flex h-screen overflow-hidden"
      style={{ background: "#0F1115", color: "#F8FAFC" }}
    >
      {/* Sidebar */}
      <HQSidebar />
      ...
      <div className="flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        >
          <div className="mx-auto max-w-[1300px] space-y-7">
            ...
            >
              <div className="pointer-events-none absolute -right-20 -top-20 h-72 w-72 rounded-full blur-3xl opacity-[0.22]" style={{ background: "#E53935" }} />
              <div className="pointer-events-none absolute -bottom-20 left-16 h-56 w-56 rounded-full blur-3xl opacity-[0.14]" style={{ background: "#8B5CF6" }} />
              <div className="relative">
                <div className="mb-5 inline-flex items-center gap-2 rounded-full px-3 py-1.5" style={{ background: "rgba(229,57,53,0.14)", border: "1px solid rgba(229,57,53,0.28)" }}>
                  <Zap size={11} color="#E53935" fill="#E53935" />
                  <span className="text-[10px] font-black tracking-[0.18em] uppercase" style={{ color: "#E53935" }}>STRONG BOXING HQ</span>
                </div>
                <h1 className="text-3xl font-black leading-tight tracking-tight md:text-4xl" style={{ color: "#F8FAFC", fontSize: undefined }}>
                  안녕하세요, 수지님 👋
                </h1>
            ...
            {/* Quick Menu */}
            <section>
              <p className="mb-4 text-[10px] font-black uppercase tracking-[0.22em]" style={{ color: "#94A3B8" }}>Quick Access</p>
              <QuickMenu />
            </section>
            ...
            <div className="grid gap-5 lg:grid-cols-3">
              ...
              <Panel className="p-6">
                <div className="mb-6 flex items-center justify-between">
                  ...
                  <span className="rounded-full px-2.5 py-0.5 text-[11px] font-black" style={{ background: "rgba(16,185,129,0.12)", color: "#10B981" }}>
                  ...
                  {TODOS.map((t, i) => (
                    <li key={i} className="flex cursor-pointer items-center gap-3">
                      {t.done
                        ...
                        : <Circle size={17} color="#2A313C" strokeWidth={2} className="shrink-0" />
                      }
                      <span className="text-[13px]" style={{ color: t.done ? "#94A3B8" : "#F8FAFC", textDecoration: t.done ? "line-through" : "none" }}>
                        {t.text}
                      </span>
                    </li>
                  ))}
                </ul>
                <button className="mt-6 flex items-center gap-1.5 text-[12px] font-bold" style={{ color: "#94A3B8", background: "none", boxShadow: "none", border: "none" }}>
              ...
              <Panel className="p-6">
                ...
                <ul className="space-y-2.5">
                  {NOTICES.map((n, i) => (
                    <li key={i} className="flex items-start gap-3 rounded-2xl p-3.5" style={{ border: "1px solid #2A313C", background: "rgba(255,255,255,0.02)" }}>
                      <span className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: n.dot }} />
                      <div>
                        <p className="text-[13px] font-semibold" style={{ color: "#F8FAFC" }}>{n.text}</p>
                        <p className="mt-1 text-[11px]" style={{ color: "#94A3B8" }}>{n.date}</p>
                      </div>
                    </li>
                  ))}
                </ul>
                <button className="mt-5 flex items-center gap-1.5 text-[12px] font-bold" style={{ color: "#94A3B8", background: "none", boxShadow: "none", border: "none" }}>
                  전체 보기 <ArrowRight size={12} />
                </button>
              </Panel>
                ...
                <ul className="space-y-4">
                  {ACTIVITIES.map((a, i) => (
                    <li key={i} className="flex items-start gap-3">
                      <span className="mt-0.5 shrink-0 rounded-xl px-2.5 py-0.5 text-[10px] font-black" style={{ background: `${a.color}18`, color: a.color }}>
                        {a.branch}
                      </span>
                      <p className="flex-1 min-w-0 text-[13px]" style={{ color: "#F8FAFC" }}>{a.text}</p>
